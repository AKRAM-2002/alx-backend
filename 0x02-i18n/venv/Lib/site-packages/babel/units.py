...
def _find_unit_pattern(unit_id: str, locale: Locale |
                       str | None = LC_NUMERIC) -> str | None:
    """
...
def format_unit(
    ...
    *,
    ...
    # nb: This is marked as no-cover, as the current CLDR seemingly has no way
    # for this to happen.
    fallback_name = get_unit_name(
        measurement_unit,
        length=length,
        locale=locale)  # pragma: no cover
    return f"{formatted_value} {
        fallback_name or measurement_unit}"  # pragma: no cover
...
def _find_compound_unit(
    ...
    resolved_denominator_unit = _find_unit_pattern(
        denominator_unit, locale=locale)

    # If either was not found, we can't possibly build a suitable compound
    # unit either.
    ...
    *,
    ...
    if isinstance(denominator_value, str):  # Denominator is preformatted
        ...
        if denominator_value == 1:  # support perUnitPatterns when the denominator is 1
            ...
            # this will make `format_unit` return " second" instead of "1
            # second".
            denominator_value = ""
    ...
    # TODO: this doesn't support "compound_variations" (or "prefix"), and will
    # fall back to the "x/y" representation
