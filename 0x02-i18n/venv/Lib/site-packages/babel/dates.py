...
if TYPE_CHECKING:
    ...
    _PredefinedTimeFormat: TypeAlias = Literal['full',
                                               'long', 'medium', 'short']
...
def _get_dt_and_tzinfo(
        dt_or_tzinfo: _DtOrTzinfo) -> tuple[datetime.datetime | None, datetime.tzinfo]:
    """
...
def _get_datetime(instant: _Instant) -> datetime.datetime:
    ...
    if instant is None:
        ...
        return datetime.datetime.fromtimestamp(
            instant, UTC).replace(tzinfo=None)
...
def _ensure_datetime_tzinfo(
        ...
        tzinfo: datetime.tzinfo | None = None) -> datetime.datetime:
    """
...
def get_period_names(width: Literal['abbreviated',
                                    'narrow',
                                    'wide'] = 'wide',
...
def get_day_names(width: Literal['abbreviated',
                                 'narrow',
                                 'short',
                                 'wide'] = 'wide',
...
def get_month_names(width: Literal['abbreviated',
                                   'narrow',
                                   'wide'] = 'wide',
                    context: _Context = 'format',
                    locale: Locale | str | None = LC_TIME) -> LocaleDataDict:
...
def get_quarter_names(width: Literal['abbreviated',
                                     'narrow',
                                     'wide'] = 'wide',
                      context: _Context = 'format',
                      locale: Locale | str | None = LC_TIME) -> LocaleDataDict:
...
def get_date_format(format: _PredefinedTimeFormat = 'medium',
                    locale: Locale | str | None = LC_TIME) -> DateTimePattern:
...
def get_time_format(format: _PredefinedTimeFormat = 'medium',
                    locale: Locale | str | None = LC_TIME) -> DateTimePattern:
...
def get_timezone_location(
    ...
    if not return_city and territory and len(
            get_global('territory_zones').get(
            territory, [])) == 1:
...
def format_timedelta(delta: datetime.timedelta | int,
...
def format_interval(
    ...
    if skeleton not in interval_formats or not skeleton:
        ...
        if not skeleton:  # Still no match whatsoever?
            ...
            return _format_fallback_interval(
                start, end, skeleton, tzinfo, locale)
...
class DateTimeFormat:
    ...
    def format_timezone(self, char: str, num: int) -> str:
        ...
        if char == 'z':
            ...
            if num == 5:
                return get_timezone_gmt(
                    value, width, locale=self.locale, return_z=True)
        ...
        # Included additional elif condition to add support for 'Xx' in
        # timezone format
        elif char == 'X':
            if num == 1:
                return get_timezone_gmt(
            ...
            if num == 1:
                ...
                return get_timezone_gmt(
                    value, width='short', locale=self.locale)
            elif num in (3, 5):
                return get_timezone_gmt(
                    value, width='iso8601', locale=self.locale)
    ...
    for tok_type, tok_value in tokenize_pattern(pattern):
        if tok_type == "chars":
            ...
            if limit and fieldnum not in limit:
                raise ValueError(
                    f"Invalid length for field: {
                        fieldchar * fieldnum!r}")
...
def untokenize_pattern(
        tokens: Iterable[tuple[str, str | tuple[str, int]]]) -> str:
    """
    ...
    for tok_type, tok_value in tokens:
        if tok_type == "field":
            ...
            if not any(
                    ch in PATTERN_CHARS for ch in tok_value):  # No need to quote
...
def match_skeleton(
    ...
    # Filter out falsy values and sort for stability; when `interval_formats`
    # is passed in, there may be a None key.
    ...
    for option in options:
        ...
        # We lost the option along the way (probably due to
        # "allow_different_fields")
        if not option:
            continue
