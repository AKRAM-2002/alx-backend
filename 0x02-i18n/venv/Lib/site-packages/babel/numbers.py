...
def validate_currency(currency: str, locale: Locale |
                      str | None = None) -> None:
...
def normalize_currency(currency: str, locale: Locale |
...
def get_currency_symbol(currency: str, locale: Locale |
    ...
    *,
    ...
    try:
        ...
        raise UnsupportedNumberingSystemError(
            f"Unknown numbering system {numbering_system} for Locale {parsed_locale}.") from error
...
def get_decimal_symbol(
    ...
    *,
    ...
    return _get_number_symbols(
        locale, numbering_system=numbering_system).get(
        'decimal', '.')
...
def format_number(number: float | decimal.Decimal | str,
...
def get_decimal_precision(number: decimal.Decimal) -> int:
    ...
    # Note: DecimalTuple.exponent can be 'n' (qNaN), 'N' (sNaN), or 'F'
    # (Infinity)
    if not isinstance(
            decimal_tuple.exponent,
            int) or decimal_tuple.exponent >= 0:
        return 0
...
def _get_compact_format(
    ...
    if number.is_nan() or number.is_infinite():
        return number, None
    format = None
    for magnitude in sorted([int(m)
                            for m in compact_format["other"]], reverse=True):
        if abs(number) >= magnitude:
            ...
            number = cast(decimal.Decimal, number /
                          (magnitude // (10 ** (pattern.count("0") - 1))))
    ...
    if format:
        ...
        try:
            ...
            raise UnknownCurrencyFormatError(
                f"{format_type!r} is not a known currency format type") from None
...
def format_compact_currency(
    ...
    *,
    ...
    try:
        ...
        raise UnknownCurrencyFormatError(
            f"{format_type!r} is not a known compact currency format type") from error
    number, format = _get_compact_format(
        number, compact_format, locale, fraction_digits)
    # Did not find a format, fall back.
...
def parse_decimal(
    ...
    *,
    ...
    locale = Locale.parse(locale)
    group_symbol = get_group_symbol(locale, numbering_system=numbering_system)
    decimal_symbol = get_decimal_symbol(
        locale, numbering_system=numbering_system)
    ...
    try:
        ...
        raise NumberFormatError(
            f"{string!r} is not a valid decimal number") from exc
...
def _remove_trailing_zeros_after_decimal(
        string: str, decimal_symbol: str) -> str:
    """
...
class NumberPattern:
    ...
    def scientific_notation_elements(
        ...
        *,
        ...
        exp_sign = ''
        if exp < 0:
            exp_sign = get_minus_sign_symbol(
                locale, numbering_system=numbering_system)
        elif self.exp_plus:
            exp_sign = get_plus_sign_symbol(
                locale, numbering_system=numbering_system)
    ...
    def apply(
        ...
        *,
        ...
        # Is it a significant digits pattern?
            ...
            if sep:
                number += get_decimal_symbol(locale,
                                             numbering_system=numbering_system) + b
    ...
    def _format_frac(
        ...
        *,
        ...
        while len(value) > min and value[-1] == '0':
            value = value[:-1]
        return get_decimal_symbol(
            locale, numbering_system=numbering_system) + value
