...
def extract_operands(
        source: float | decimal.Decimal) -> tuple[decimal.Decimal | int, int, int, int, int, int, Literal[0], Literal[0]]:
...
class PluralRule:
    ...
    def parse(cls,
              rules: Mapping[str,
                             str] | Iterable[tuple[str,
                                                   str]] | PluralRule) -> PluralRule:
...
def to_javascript(rule: Mapping[str, str] |
                  Iterable[tuple[str, str]] | PluralRule) -> str:
...
def to_gettext(rule: Mapping[str, str] |
               Iterable[tuple[str, str]] | PluralRule) -> str:
...
def in_range_list(num: float | decimal.Decimal,
                  range_list: Iterable[Iterable[float | decimal.Decimal]]) -> bool:
...
def within_range_list(num: float | decimal.Decimal,
                      range_list: Iterable[Iterable[float | decimal.Decimal]]) -> bool:
...
_VARS = {
    ...
    # compact decimal exponent value: exponent of the power of 10 used in
...
def tokenize_rule(s: str) -> list[tuple[str, str]]:
    ...
    while pos < end:
        for tok, rule in _RULES:
            ...
            raise RuleError(
                f"malformed CLDR pluralization rule.  Got unexpected {
                    s[pos]!r}")
    return result[::-1]
...
def skip_token(tokens: list[tuple[str, str]],
               type_: str, value: str | None = None):
...
def negate(rv: tuple[Any, ...]) -> tuple[Literal['not'],
                                         tuple[tuple[Any, ...]]]:
    return 'not', (rv,)
...
class _Parser:
    ...
    def __init__(self, string):
        ...
        if self.tokens:
            raise RuleError(
                f"Expected end of rule, got {self.tokens[-1][1]!r}")
    ...
    def relation(self):
        ...
        if skip_token(self.tokens, 'word', 'is'):
            return skip_token(
                self.tokens, 'word', 'not') and 'isnot' or 'is', (left, self.value())
...
def _binary_compiler(tmpl):
    ...
    return lambda self, left, right: tmpl % (
    ...
    def compile_relation(self, method, expr, range_list, negated=False):
        ...
        for item in range_list[1]:
            if item[0] == item[1]:
                ...
                ranges.append(
                    f"{self.compile(item[0])}..{self.compile(item[1])}")
        return f"{
            self.compile(expr)}{
            ' not' if negated else ''} {method} {
            ','.join(ranges)}"
