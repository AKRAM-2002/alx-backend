    ...
    def run(self):
        ...
        with open(self.output_file, 'wb') as outfile:
            catalog = Catalog(
                ...
                header_comment=(
                    self.header_comment or DEFAULT_HEADER),
        ...
        if self.mapping_file:
            if self.mapping_file.endswith(".toml"):
                with open(self.mapping_file, "rb") as fileobj:
                    file_style = (
                        "pyproject.toml" if os.path.basename(
                            self.mapping_file) == "pyproject.toml" else "standalone")
...
class InitCatalog(CommandMixin):
    ...
    def run(self):
        self.log.info('creating catalog %s based on %s',
...
def _parse_config_object(config: dict, *, filename="(unknown)"):
    ...
    if not isinstance(extractors_read, dict):
        raise ConfigurationError(
            f"{filename}: extractors: Expected a dictionary, got {
                type(extractors_read)!r}")
    for method, callable_spec in extractors_read.items():
        if not isinstance(method, str):
            # Impossible via TOML, but could happen with a custom object.
            raise ConfigurationError(
                f"{filename}: extractors: Extraction method must be a string, got {
                    method!r}")
        if not isinstance(callable_spec, str):
            raise ConfigurationError(
                f"{filename}: extractors: Callable specification must be a string, got {
                    callable_spec!r}")
    ...
    if not isinstance(mappings_read, list):
        raise ConfigurationError(
            f"{filename}: mappings: Expected a list, got {
                type(mappings_read)!r}")
    for idx, entry in enumerate(mappings_read):
        if not isinstance(entry, dict):
            raise ConfigurationError(
                f"{filename}: mappings[{idx}]: Expected a dictionary, got {
                    type(entry)!r}")
        entry = entry.copy()
        ...
        if not isinstance(method, str):
            raise ConfigurationError(
                f"{filename}: mappings[{idx}]: 'method' must be a string, got {
                    method!r}")
        ...
        if not isinstance(pattern, (list, str)):
            raise ConfigurationError(
                f"{filename}: mappings[{idx}]: 'pattern' must be a list or a string, got {
                    pattern!r}")
        ...
        for pat in pattern:
            if not isinstance(pat, str):
                raise ConfigurationError(
                    f"{filename}: mappings[{idx}]: 'pattern' elements must be strings, got {
                        pat!r}")
...
def _parse_mapping_toml(
    ...
    try:
        ...
        try:
            ...
            raise ImportError(
                "tomli or tomllib is required to parse TOML files") from ie

    try:
        parsed_data = tomllib.load(fileobj)
    except tomllib.TOMLDecodeError as e:
        raise ConfigurationError(
            f"{filename}: Error parsing TOML file: {e}") from e
    ...
    if style == "pyproject.toml":
        try:
            ...
            raise ConfigurationError(
                f"{filename}: No 'tool.babel' section found in file") from e
...
def _parse_spec(s: str) -> tuple[int | None,
