...
if TYPE_CHECKING:
    ...
    _SimpleKeyword: TypeAlias = tuple[int |
    ...
    _FileExtractionResult: TypeAlias = tuple[str, int,
                                             str | tuple[str, ...], list[str], str | None]

    # 4-tuple of (lineno, message, comments, context)
    _ExtractionResult: TypeAlias = tuple[int,
                                         str | tuple[str, ...], list[str], str | None]
...
def _match_messages_against_spec(lineno: int,
                                 messages: list[str | None],
                                 comments: list[str],
                                 fileobj: _FileObj,
                                 spec: tuple[int | tuple[int,
                                                         str],
                                             ...]):
    translatable = []
    context = None
    ...
    for tok, value, (lineno, _), _, _ in tokens:
        if call_stack == -1 and tok == NAME and value in ('def', 'class'):
            ...
            if (tok == OP and value == ')') or nested:
                ...
                messages = tuple(messages) if len(
                    messages) > 1 else messages[0]
...
def _parse_python_string(
    ...
    if isinstance(code, ast.Expression):
        ...
        if isinstance(body, ast.JoinedStr):  # f-string
            ...
            # TODO: we could raise an error or warning when not all nodes are
            # constants
    return None
...
def extract_javascript(
    ...
    for token in tokenize(
        ...
        if options.get(
                'parse_template_string') and not funcname and token.type == 'template_string':
