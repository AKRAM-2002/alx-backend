    ...
    *,
    ...
    for message in _sort_messages(catalog, sort_by=sort_by):
        ...
        if not no_location:
            ...
            try:
                locations = sorted(
                    message.locations, key=lambda x: (
                        x[0], isinstance(
                            x[1], int) and x[1] or -1))
        ...
        if message.previous_id and include_previous:
            ...
            if len(message.previous_id) > 1:
                norm_previous_id = normalize(
                    message.previous_id[1], width=width)
...
def _sort_messages(messages: Iterable[Message],
                   sort_by: Literal["message",
                                    "location"] | None) -> list[Message]:
    """
